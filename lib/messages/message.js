...
typeMap[ASN1.Ber.OID] = { type: 'oid', parse: parseOID };

function parseInt(buf)
{
	var fb = buf[0];
	var value = new BigInt(fb & 0x7f);

	for (var i = 1; i < buf.length; i++) {
		value = value.shiftLeft(8).add(buf[i] & 0xff);
	}

	return (fb & 0x80) ? value.mul(-1) : value;
}
var intTypes = [ ASN1.Ber.Integer, 66, 67 ];
...
SNMPMessage.prototype.parse = function parse(buf)
{
	...
	while (ber.offset < end) {
		...
		varBind.oid = parseOID(ber.readString(ASN1.Ber.OID, true));
		varBind.tag = ber.peek();
		varBind.raw = ber.readString(varBind.tag, true);

		var f = typeMap[varBind.tag];
		if (typeof(f) === 'undefined') {
			varBind.type = 'unknown';
			varBind.value = varBind.raw;
		} else {
			varBind.type = f.type;
			varBind.value = f.parse(varBind.raw);
		}
