...
function nextBulkGet(self, options, varBindList)
{
	...
	for (var i = options.nonRepeaters; i < options.oids.length; i++) {
		...
		if (typeof(varBind) !== 'undefined' &&
		    cmpOIDPrefix(options.oids[i], varBind.oid))
	...
	do {
		...
		key = id.toString(16);
	} while (typeof(self._requests[key]) !== 'undefined');
	...
	options.ber = ber;
	self._requests[key] = options;
	options.timeout = setTimeout(managerRetry, options.delay, self, key);
...
{
	var request = self._requests[key];
...
function managerRecv(self, buf, peer)
{
	...
	var key = msg.messageId.toString(16);
...
Manager.prototype.get = function (agent, oids, cb, options)
{
	...
	do {
		id = Math.floor(Math.random() * (1 << 30));
		key = id.toString(16);
	} while (typeof(this._requests[key]) !== 'undefined');
